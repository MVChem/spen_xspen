...
from data_v2 import PriorData
from model_v2 import StrongPrior, sample_prior
from prepare_data import HERE,V1,sha256
from train import save_checkpoint,save_grid,validate
...
def main():
    ...
    if rank==0:
        ...
        for f in ['train_strong.py','data_v2.py','model_v2.py','tiny_unet_v2.py','prepare_data.py','prepare_mixed.py','download_mouse.py','run_guard.py']:
            shutil.copyfile(HERE/f,snapshot/f)
        for f in ['model.py','train.py']:
            shutil.copyfile(V1/f,snapshot/f'v1_{f}')
        ...
        if not args.resume:
            ...
            old,_=load_prior(args.reference_checkpoint,device)
            old_loss=validate(old,val);del old
            (args.out/'baseline.json').write_text(json.dumps(dict(initial_model=initial_loss,v1_same_validation=old_loss))+'\n')
