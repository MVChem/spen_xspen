...
> 本目录是 2026-09-14 迁入新工程的 SPENPy 1.0.0 源码副本，由项目根 Git 统一管理。
> 本地开发使用项目根目录的共享 `.venv`，安装本目录的源码，依赖声明见 [pyproject.toml](pyproject.toml)。以下保留上游 API 说明；数据、示例结果和旧参考实现仍在 [旧目录](/home/data2/chk/workspace/2026/08/14/01_工作项目/spenpy/)。示例运行需显式提供输入文件；无数据检查使用 `pytest -m "not integration"`。
...
- [comparison.png](/home/data2/chk/workspace/2026/08/14/01_工作项目/spenpy/outputs/comparison.png): rows show SPEN180, xSPEN, and Hybrid SPEN-Diff2; columns show the input MRI, RO FFT + RSS, CG, and PhaseMap + InvA.
- [hybrid_spen_diffusion.png](/home/data2/chk/workspace/2026/08/14/01_工作项目/spenpy/outputs/hybrid_spen_diffusion.png): RO-only and reconstructed images for Hybrid SPEN-Diff2 b0 / DWI-RO / DWI-PE / DWI-SS volumes. Simulation parameters are also saved as JSON.

Both examples use a public MRHead 256×256 magnitude slice as the input phantom. The image is encoded directly onto the acquisition grid without first downsampling it. [MRI data provenance](/home/data2/chk/workspace/2026/08/14/01_工作项目/spenpy/data/mri/README.md) includes verification details. The Hybrid example assigns this array to the protocol FOV; it does not reproduce the MID253 subject's anatomy, contrast, or diffusion tensor.
